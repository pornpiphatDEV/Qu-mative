...
def quizResult(request, pk):
    ...
    return render(request, 'quiz/result.html', {'quiz': quiz, 'takers': takers, 'pk': pk})


def Export_resultlist(request, pk):
    quiz = get_object_or_404(Quiz, id=pk)
    ...
    for std in takers:
        ...
        print(rownumber)
        columns = [std.name,std.email,std.score]
        for col in range(len(columns)):
            print(col)
            worksheet.write(rownumber , col , columns[col])
