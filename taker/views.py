...
def result(request, pk, taker_id):
    ...
    return render(request, 'taker/result.html', {'score': score, 'quiz': quiz, 'questions': takerQuestions, 'assessteaching': assessteaching, 'mistakesassessteaching': mistakesassessteaching, "quantityexams": quantityexams, 'pk': pk, 'taker_id': taker_id})


def Export_resultlist(request, pk, taker_id):
    quiz = get_object_or_404(Quiz, id=pk)
    taker = get_object_or_404(Taker, id=taker_id)
    takerQuestions = createTakerQuestionsList(quiz.question_set.all(), taker)
    ...
    response["content-Disposition"] = 'attachment ; filename=Takers' + \
        str(datetime.datetime.now())+'.xls'
    workbook = xlwt.Workbook(encoding='utf-8')
    worksheet = workbook.add_sheet("Takers")
    columns = ['ด้านพุทธิพิสัย', 'จำนวนข้อที่ถูกต้อง', 'จำนวนข้อที่ผิด']
    rownumber = 0
    for col in range(len(columns)):
        worksheet.write(rownumber, col, columns[col])

    worksheet.write(1, 0, 'จำ')
    worksheet.write(1, 1, assessteaching['Remember'])
    worksheet.write(1, 2, mistakesassessteaching['Remember'])

    worksheet.write(2, 0, 'เข้าใจ')
    worksheet.write(2, 1, assessteaching['Understand'])
    worksheet.write(2, 2, mistakesassessteaching['Understand'])

    worksheet.write(3, 0, 'ประยุกต์ใช้')
    worksheet.write(3, 1, assessteaching['Apply'])
    worksheet.write(3, 2, mistakesassessteaching['Apply'])

    worksheet.write(4, 0, 'วิเคราะห์')
    worksheet.write(4, 1, assessteaching['Analyze'])
    worksheet.write(4, 2, mistakesassessteaching['Analyze'])

    worksheet.write(5, 0, 'ประเมินค่า')
    worksheet.write(5, 1, assessteaching['Evaluate'])
    worksheet.write(5, 2, mistakesassessteaching['Evaluate'])

    worksheet.write(6, 0, 'สร้างสรรค์')
    worksheet.write(6, 1, assessteaching['Creative'])
    worksheet.write(6, 2, mistakesassessteaching['Creative'])

    worksheet.write(8, 0, 'ผู้ทำแบบทดสอบ')
    worksheet.write(8, 1, takername)
